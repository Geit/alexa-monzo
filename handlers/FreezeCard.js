      ...
      if (cards[0].status === 'ACTIVE') {
        monzoUser.setCardState(cards[0].id, false).then(() => {
          if (!this.event.session.new)
            this.emit(':ask', `${t(this.locale, 'CardFrozen')} ${t(this.locale, 'ContinueSessionPrompt')}`);
          else
            this.emit(':tell', t(this.locale, 'CardFrozen'));
        });
      } else {
        if (!this.event.session.new)
          this.emit(':ask', `${t(this.locale, 'CardAlreadyFrozen')} ${t(this.locale, 'ContinueSessionPrompt')}`);
        else
