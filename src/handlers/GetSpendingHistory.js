...
export default function GetSpendingHistoryIntent() {
  ...
  monzoUser
    ...
    .then(accounts => {
      ...
      if (
        this.event.request.intent.slots.TransactionCategory &&
        this.event.request.intent.slots.TransactionCategory.value
      )
        category = this.event.request.intent.slots.TransactionCategory.value;

      monzoUser
        .getTransactions(accounts[0].id, { since: timePeriod.format('YYYY-MM-DD[T]HH:mm:ss[Z]') })
        .then(transactions => {
          let sumOfSpending = 0;
          ...
          if (sumOfSpending > 0) {
            if (category)
              response = t(this.locale, 'SpendingTotalWithCategory', {
                amount: utils.currencyToWords(sumOfSpending),
                category,
                duration: duration.humanize().replace('a ', ''),
              });
            else
              response = t(this.locale, 'SpendingTotal', {
                amount: utils.currencyToWords(sumOfSpending),
                duration: duration.humanize().replace('a ', ''),
              });
          } else if (category) {
            response = t(this.locale, 'NoSpendingWithCategory', {
              category,
              duration: duration.humanize().replace('a ', ''),
            });
          } else {
