...
module.exports = {
  monzoUser(alexaRequest) {
    ...
    this.getAccounts = function() {
      if (
        alexaRequest.attributes &&
        alexaRequest.attributes.accounts &&
        moment(alexaRequest.attributes.accounts.lastFetch).isAfter(CACHE_TIMEOUTS.ACCOUNT)
      )
        return new Promise((resolve, reject) => {
          resolve(alexaRequest.attributes.accounts.slice());
    ...
    this.getCards = function(accountId) {
      if (
        alexaRequest.attributes.cards &&
        moment(alexaRequest.attributes.cards.lastFetch).isAfter(CACHE_TIMEOUTS.CARDS)
      )
        return new Promise((resolve, reject) => {
          resolve(alexaRequest.attributes.cards.slice());
        });
